...
class CF_Authentication {
	...
	public function __construct(
		...
		$this->cfs_http = new CF_Http( CF_Constants::DEFAULT_CF_API_VERSION );
	}
	 ...
	 */
	public function authenticate( $version = CF_Constants::DEFAULT_CF_API_VERSION ) {
...
class CF_Connection {
	 ...
	 */
	public function __construct( CF_Authentication $cfs_auth, $servicenet = False ) {
		...
		if ( !$this->cfs_auth->authenticated() ) {
			$e = "Need to pass in a previously authenticated CF_Authentication instance.";
	...
	public function create_container( $container_name = NULL ) {
		...
		} elseif ( strlen( $container_name ) > CF_Constants::MAX_CONTAINER_NAME_LEN ) {
			throw new SyntaxException( sprintf(
				"Container name exeeds %d bytes.", CF_Constants::MAX_CONTAINER_NAME_LEN ) );
		}
		...
		if ( $return_code != 201 && $return_code != 202 ) {
			throw new InvalidResponseException( "Invalid response (" . $return_code . "): "
...
class CF_Container {
	 ...
	 */
	public function __construct(
		CF_Authentication $cfs_auth, CF_Http $cfs_http,
		$name, $count = 0, $bytes = 0, $docdn = True
	) {
		if ( strlen( $name ) > CF_Constants::MAX_CONTAINER_NAME_LEN ) {
			throw new SyntaxException( "Container name exceeds maximum allowed length." );
		} elseif ( strpos( $name, "/" ) !== False ) {
			throw new SyntaxException( "Container names cannot contain a '/' character." );
		}
		$this->cfs_auth = $cfs_auth;
	 ...
	 */
	public function create_object( $obj_name = NULL, $hdrs = array( ) ) {
	...
	/**
	 * @see CF_Container::move_object_to()
	 * @return CF_Async_Op
	 */
...
class CF_Object {
	 ...
	 */
	public function __construct(
		CF_Container $container, $name, $force_exists = False, $dohead = True, $hdrs = array()
	) {
		if ( $name[0] == "/" ) {
			$r = "Object name '" . $name;
		...
		if ( strlen( $name ) > CF_Constants::MAX_OBJECT_NAME_LEN ) {
			throw new SyntaxException( "Object name exceeds maximum allowed length." );
		}
	...
	function _guess_content_type( $handle ) {
		...
		if ( function_exists( "finfo_open" ) ) {
			...
			if ( $finfo ) {
				if ( is_file( (string) $handle ) ) { // @FIXME: WTF is this shit?
		...
		if ( !$this->content_type && (string) is_file( $handle ) // @FIXME: WTF is this shit?
	 ...
	 */
	public function public_ssl_uri() {
		if ( $this->container->cdn_enabled ) {
	 ...
	 */
	public function public_streaming_uri() {
		if ( $this->container->cdn_enabled ) {
	...
	public function write( $data = NULL, $bytes = 0, $verify = True ) {
		return $this->write_internal( 'sync', $data, $bytes, $verify );
	}
	 ...
	 * @return CF_Async_Op
	 */
	public function write_async( $data = NULL, $bytes = 0, $verify = True ) {
		return $this->write_internal( 'async', $data, $bytes, $verify );
	}
	...
	public function load_from_filename( $filename, $verify = True ) {
		...
		if ( $size > CF_Constants::MAX_OBJECT_SIZE ) {
	...
	public function purge_from_cdn( $email = null ) {
		...
		$status = $this->container->cfs_http->purge_from_cdn(
			$this->container->name . "/" . $this->name, $email );
		if ( $status < 199 or $status > 299 ) {
			throw new InvalidResponseException(
	 ...
	 * @return string MD5 checksum hexidecimal string
	 */
	private function compute_md5sum( &$data ) {
		if ( function_exists( "hash_init" ) && is_resource( $data ) ) {
			...
			rewind( $data );
		} elseif ( (string) is_file( $data ) ) { // @FIXME: WTF is this shit?
			$md5 = md5_file( $data );
		} else {
			$md5 = md5( $data );
		}
	...
	private function _initialize( $hdrs = array( ) ) {
		...
		$this->setMetadataValues( $metadata );
